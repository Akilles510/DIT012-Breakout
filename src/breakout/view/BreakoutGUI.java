...
public class BreakoutGUI extends Application implements IEventHandler {
        ...
        switch (kc) {
            case LEFT:
                if(paddle.canMoveToPositionLeft()){
                    paddle.updatePaddleXPosition(-Paddle.MOVEMENT_IN_X_AXIS);
                }
                break;
            case RIGHT:
                if(paddle.canMoveToRight()){
                    paddle.updatePaddleXPosition(Paddle.MOVEMENT_IN_X_AXIS);
                }
                break;
            default:  // Nothing
    ...
    private List<Wall> getWalls() {
        Wall left = new Wall(Wall.Dir.VERTICAL, 0);
        Wall top = new Wall(Wall.Dir.HORIZONTAL, 0);
        Wall right = new Wall(Wall.Dir.VERTICAL,400);
    ...
    private List<Brick> getBricks(int nRows, int nCols) {
        ...
        for (int y = 10 * offset; y < nRows * (bh + offset); y += bh + offset) {
            for (int x = offset - 2; x < nCols * (bw + offset); x += bw + offset) {
                Brick b = new Brick();
                b.setX(x);
                b.setX2(x + BRICK_WIDTH);
                b.setY(y);
                b.setY2(y + BRICK_HEIGHT);
                b.points = points;
                bricks.add(b);
            }
            points -= 100;
